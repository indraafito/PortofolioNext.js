-- Neon Postgres migration (Supabase-free)
-- This script adapts your existing schema for Neon by:
-- - Removing Supabase auth dependencies (auth.users, auth.uid())
-- - Using pgcrypto's gen_random_uuid() for UUIDs
-- - Disabling RLS (enforce access in your app/backend)
-- You can run this in Neon SQL Editor or via psql.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_education_updated_at'
  ) THEN
...
-- Disable RLS (handled by app/backend)
